...
import { scgUrl, SPINER_COLOR } from '@constants';
...
export const Scg: FC<IProps> = ({ action, className, show = false }) => {
  ...
  const onCommandExecuted = useCallback(
    (data: IWindowEventData) => {
      ...
      scNavigation.goToScnAction(String(data.payload.response.action));
    },
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!activeLangKeynode?.value) return;
      removeFromCache(action);
      iframe.contentWindow?.postMessage(
        { type: 'renderScg', addr: Number(action), lang: activeLangKeynode.value },
        '*',
      );
    })();
    ...
    <>
      ...
      <div className={classNames(styles.wrap, show && styles.wrapShow, className)}>
        <button type="button" className={styles.reloadButton} onClick={handleReload}>
          {translate({ ru: 'Перезагрузить SCG', en: 'Reload SCG' })}
        </button>
